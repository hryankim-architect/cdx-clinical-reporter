...
# defaults. On a fresh checkout without the substrate present, run `make run`.

set -euo pipefail

export AUDIT_HOST="${AUDIT_HOST:-chi-mac-m:8081}"
export MLFLOW_TRACKING_URI="${MLFLOW_TRACKING_URI:-http://chi-mac-m:5050}"
export CDXREPORT_RUN_NAME="${CDXREPORT_RUN_NAME:-lab-$(date -u +%Y%m%d-%H%M%S)}"
...
echo "[run_lab] RUN_NAME=${CDXREPORT_RUN_NAME}"

uv run make run RUN_NAME="${CDXREPORT_RUN_NAME}"

echo "[run_lab] canary check"
uv run python -m cdxreport.canary
