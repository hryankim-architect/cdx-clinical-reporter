...
def test_canary_passes_on_default_fixture(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CDXREPORT_CANARY_FIXTURE", str(fixture_dst))

    result = canary.check()
    assert result["ok"], result
    assert result["intact"] and result["tamper_detected"]
...
def test_canary_fails_on_missing_keys(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("CDXREPORT_CANARY_FIXTURE", str(fixture_path))
    monkeypatch.chdir(tmp_path)
    result = canary.check()
